...
const encryptTextWithDerivedKey = watt(function* (
  ...
  const encryptedTextBytes = yield encryptTextEx(
    plainText,
    derivedKey,
    keySize,
    ivSize,
  ...
  return Buffer.concat([salt, encryptedTextBytes]);
});
...
function decryptBytesWithDerivedKey(
  ...
  const cipherTextBytesWithSaltAndIv = encryptedBytes;
  // Get the saltbytes by extracting the first keySize bytes from the supplied cipherText bytes.
  const salt = Buffer.from(cipherTextBytesWithSaltAndIv.slice(0, keySize));
...
function decryptBytesEx(
  ...
  const cipherTextBytesWithIv = encryptedBytes;
  // Get the IV bytes by extracting the next ivSize bytes from the supplied cipherText bytes.
  const iv = Buffer.from(cipherTextBytesWithIv.slice(0, ivSize));
  // Get the actual cipher text bytes by removing the first 64 bytes from the cipherText string.
  const cipherTextBytes = Buffer.from(cipherTextBytesWithIv.slice(ivSize));

  // TODO: support other cipher and padding modes

  // The cipher-block chaining mode of operation maintains internal
  // state, so to decrypt a new instance must be instantiated.
  const aesCbc = new aesjs.ModeOfOperation.cbc(Buffer.from(keyBytes), iv);
