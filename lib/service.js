...
const {decryptBytes, encryptText, decryptStream} = require('./crypto.js');
const {publicMethods, readEncryptionKey} = require('./utils.js');
    ...
    .forEach((questName) => {
      ...
      const handleRequest = watt(function* (request) {
        try {
          ...
          if (secure) {
            // Delete temp file after it has been used by request
            fs.rmSync(request.body.filePath);
          }
        }
      });
      ...
      fastify.route({
        ...
        handler: watt(function* (request, reply, next) {
          ...
          if (secure) {
            // Encrypt and send payload response as buffer
            reply.header('Content-Type', 'application/x-protobuf');
            payload = yield encryptText(
