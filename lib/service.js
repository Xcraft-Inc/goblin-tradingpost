      ...
      fastify.route({
        ...
        handler: watt(function* (request, reply) {
          try {
            ...
            if (body) {
              log.dbg(
                `Route "/${goblinIdName}/${questName}" with body: ${JSON.stringify(
                  request.body
                )}`
              );
              response = yield goblinAPI[questName](request.body);
            }

            if (params) {
              log.dbg(
                `Route "/${goblinIdName}/${questName}" with params: ${JSON.stringify(
                  request.params
                )}`
              );
              response = yield goblinAPI[questName](request.params);
            }
