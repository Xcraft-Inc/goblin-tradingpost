...
Goblin.registerQuest(goblinName, 'add-goblin-api', function (
  ...
  quest.me.change({
    ...
    newValue: {questsBlacklist, secure},
  });
  ...
  publicMethods(goblinAPI)
    ...
    .forEach((questName) => {
      ...
      log.dbg(`register quest '${goblinIdName}/${questName}' to HTTP server`);
      ...
      fastify.put(
        ...
        watt(function* (request, reply, next) {
          try {
            ...
            log.dbg(
              `Route "/${goblinIdName}/${questName}" with args: ` +
...
Goblin.registerQuest(goblinName, 'remove-goblin-api', function (
  ...
  quest.me.deleteState({path: `trading.${goblinId}`});
});

Goblin.registerQuest(goblinName, 'remove-all-goblin-api', function (quest) {
  // Need close/start  or restart
  quest.me.change({path: 'trading', newValue: {}});
});
...
Goblin.registerQuest(goblinName, 'start', function* (quest, next) {
  ...
  try {
    currentAddress = yield fastify.listen(next);
